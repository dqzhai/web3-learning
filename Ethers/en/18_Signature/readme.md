...
# Ethers: 18. Digital Signature Script
...
In this chapter, we will introduce a method of using off-chain signatures as a whitelist for NFTs. If you are not familiar with the `ECDSA` contract, please refer to [Solidity 37: Digital Signature](https://www.wtf.academy/solidity-application/Signature/).
...
The `SignatureNFT` contract in the [Solidity 37: Digital Signature](https://github.com/AmazingAng/WTF-Solidity/blob/main/37_Signature/readme.md) uses `ECDSA` to validate whitelist addresses and mint NFTs. Let's discuss two important functions:
...
4. Deploy the NFT contract using the contract factory.
    ...
    const contractNFT = await factoryNFT.deploy("Signature", "WTF", wallet.address)
